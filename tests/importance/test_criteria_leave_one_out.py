...
"""test for skcriteria.importance.criteria_leave_one_out"""
...
from skcriteria.importance import CriteriaLeaveOneOutChecker
...
def _reversal_dm():
    """2-criteria, 3-alternative dm crafted so that dropping ``C0``
    fully reverses the ranking, and dropping ``C1`` leaves it untouched.
...
def test_CriteriaLeaveOneOutChecker_dmaker_no_evaluate_method():
    class NoEvaluateMethod:
        pass

    with pytest.raises(TypeError):
        CriteriaLeaveOneOutChecker(NoEvaluateMethod())


def test_CriteriaLeaveOneOutChecker_dmaker_evaluate_not_callable():
    class EvaluateNotCallable:
        evaluate = None

    with pytest.raises(TypeError):
        CriteriaLeaveOneOutChecker(EvaluateNotCallable())


def test_CriteriaLeaveOneOutChecker_invalid_metric():
    with pytest.raises(ValueError):
        CriteriaLeaveOneOutChecker(simple.WeightedSumModel(), metric="pearson")
...
def test_CriteriaLeaveOneOutChecker_requires_at_least_two_criteria():
    dm = mkdm(
        matrix=[[1], [2], [3]],
        objectives=[max],
        alternatives=["A", "B", "C"],
    ...
    checker = CriteriaLeaveOneOutChecker(simple.WeightedSumModel())
    with pytest.raises(ValueError):
        checker.evaluate(dm)
...
def test_CriteriaLeaveOneOutChecker_ranks_names():
    dm = _reversal_dm()
    checker = CriteriaLeaveOneOutChecker(simple.WeightedSumModel())
...
def test_CriteriaLeaveOneOutChecker_importance_values(metric):
    dm = _reversal_dm()
    checker = CriteriaLeaveOneOutChecker(
...
@pytest.mark.parametrize("metric", ["footrule", "kendall"])
def test_CriteriaLeaveOneOutChecker_importance_bounded(metric):
    dm = skc.datasets.load_simple_stock_selection()
    checker = CriteriaLeaveOneOutChecker(TOPSIS(), metric=metric)
...
def test_CriteriaLeaveOneOutChecker_missing_alternative_forbidden():
    dm = skc.datasets.load_simple_stock_selection()
...
def test_CriteriaLeaveOneOutChecker_missing_alternative_allowed():
    dm = skc.datasets.load_simple_stock_selection()
    dmaker = DropAlternativeDMaker(TOPSIS(), "AA")
    checker = CriteriaLeaveOneOutChecker(
...
def test_CriteriaLeaveOneOutChecker_parallel_matches_sequential():
    dm = skc.datasets.load_simple_stock_selection()
    dmaker = TOPSIS()

    sequential = CriteriaLeaveOneOutChecker(dmaker).evaluate(dm)
    parallel = CriteriaLeaveOneOutChecker(
...
def test_CriteriaLeaveOneOutChecker_repr():
    dmaker = simple.WeightedSumModel()
    checker = CriteriaLeaveOneOutChecker(dmaker)

    result = repr(checker)
    expected = (
        f"<CriteriaLeaveOneOutChecker [allow_missing_alternatives={True}, "
